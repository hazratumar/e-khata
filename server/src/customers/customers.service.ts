...
export class CustomersService {
    ...
    async findAll(page: number, limit: number): Promise<{ customers: Customer[], total: number, page: number, totalPages: number }> {
        // Input validation
        if (page < 1) {
        ...
        if (totalPages > 0 && page > totalPages) {
            throw new Error(`Page number must be less than or equal to ${totalPages}.`);
        }
